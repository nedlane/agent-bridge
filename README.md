...
| `/harness <claude\|codex\|antigravity> [worker]` | Switch a channel between **Claude Code**, **Codex**, and **Antigravity** (the non-Claude engines run in YOLO mode — bypass all approvals + sandbox, the analog of Claude's bypass-permissions). Stops the worker; the next message starts it on the new engine. Each engine keeps its **own** conversation, so switching back resumes that engine's last thread. Antigravity is **experimental** — see [Antigravity limitations](#antigravity-limitations). |
